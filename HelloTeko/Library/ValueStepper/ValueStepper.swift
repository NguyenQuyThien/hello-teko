...
@IBDesignable open class ValueStepper: UIControl {
    ...
    @IBInspectable public var iconButtonColor: UIColor = .black

    /// The color of the +/- icons when in disabled state.
    @IBInspectable public var disabledIconButtonColor: UIColor = .lightGray
    ...
    @IBInspectable public var fontSize: CGFloat = 15 {
        didSet {
            valueLabel.font = UIFont.boldSystemFont(ofSize: fontSize)
        }
    }
    
    @IBInspectable public var enable: Bool = true {
        didSet {
            setState()
        }
    }
...
//        button.setTitle("-", for: .normal)
        ...
        button.setImage(R.image.removeCircle(), for: .normal)
        button.setTitleColor(.black, for: .normal)
        button.setTitleColor(disabledIconButtonColor, for: .disabled)
        return button
    }()
...
//        button.setTitle("+", for: .normal)
        button.imageEdgeInsets = UIEdgeInsets(top: 6, left: 6, bottom: 6, right: 6)
        button.setImage(R.image.addCircle(), for: .normal)
        button.setTitleColor(.black, for: .normal)
        button.setTitleColor(disabledIconButtonColor, for: .disabled)
        return button
    }()
    ...
    var valueDidChangeHandler: ((Double) -> Void)?
...
//        decreasePath.lineWidth = thickness
        // Horizontal + line
//        decreasePath.move(to: CGPoint(x: (sliceWidth - iconSize) / 2 + 5, y: sliceHeight / 2 + 0.5))
//        decreasePath.addLine(to: CGPoint(x: (sliceWidth - iconSize) / 2 + 5 + iconSize, y: sliceHeight / 2 + 0.5))
//        tintColor.setStroke()

        // Create layer so that we can dynamically change its color when not enabled
//        decreaseLayer.path = decreasePath.cgPath
//        decreaseLayer.lineWidth = thickness
//        decreaseLayer.strokeColor = enable ? iconButtonColor.cgColor : disabledIconButtonColor.cgColor
...
//        increasePath.lineWidth = thickness
        // Horizontal + line
//        increasePath.move(to: CGPoint(x: (sliceWidth - iconSize) / 2 + sliceWidth * 2 - 5, y: sliceHeight / 2 + 0.5))
//        increasePath.addLine(to: CGPoint(x: (sliceWidth - iconSize) / 2 + iconSize + sliceWidth * 2 - 5, y: sliceHeight / 2 + 0.5))
        // Vertical + line
//        increasePath.move(to: CGPoint(x: sliceWidth / 2 + sliceWidth * 2 - 5, y: (sliceHeight / 2) - (iconSize / 2) + 0.5))
//        increasePath.addLine(to: CGPoint(x: sliceWidth / 2 + sliceWidth * 2 - 5, y: (sliceHeight / 2) + (iconSize / 2) + 0.5))
//        tintColor.setStroke()

        // Create layer so that we can dynamically change its color when not enabled
//        increaseLayer.path = increasePath.cgPath
//        increaseLayer.lineWidth = thickness
//        increaseLayer.strokeColor = enable ? iconButtonColor.cgColor : disabledIconButtonColor.cgColor
//        layer.addSublayer(increaseLayer)
    ...
    private func setState() {
        ...
        valueLabel.textColor = enable ? labelTextColor : disabledIconButtonColor
//        increaseLayer.strokeColor = increaseButton.isEnabled ? iconButtonColor.cgColor : disabledIconButtonColor.cgColor
//        decreaseLayer.strokeColor = decreaseButton.isEnabled ? iconButtonColor.cgColor : disabledIconButtonColor.cgColor
        self.layer.borderColor = enable ? tintColor.cgColor : disabledIconButtonColor.cgColor
    }
